...
class MainWindow(QMainWindow):
    ...
    def select_camera(self, i):
        camera = QCamera(self.available_cameras[i])
        camera.setViewfinder(self.viewfinder)
        camera.setCaptureMode(QCamera.CaptureStillImage)
        camera.error.connect(lambda: self.alert(camera.errorString()))
        camera.start()

        self.capture = QCameraImageCapture(camera)
        self.capture.error.connect(lambda i, e, s: self.alert(s))
        self.capture.imageCaptured.connect(lambda: self.status.showMessage("Image %04d captured" % self.save_seq))
