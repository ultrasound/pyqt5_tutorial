...
class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        new_file_action = QAction(QIcon(os.path.join('images', 'document-plus.png')), "New file", self)
        ...
        # for color in COLOR:
        #     item = QStandardItem(str(color))
        #     item.setForeground(QColor(color))
        #     model.appendRow(item)
        # self.text_color.currentIndexChanged[str].connect(lambda color: self.editor.setTextColor(QColor(color)))
        # format_toolbar.addWidget(self.text_color)

        self.text_color_action = QAction(QIcon(os.path.join('images', 'color-pencil.png')), 'Text Color', self)
        ...
        self.text_background_color_action = QAction(QIcon(os.path.join('images', 'color.png')), 'Background Color', self)
        self.text_background_color_action.setStatusTip('Change text background color')
        self.text_background_color_action.triggered.connect(self.change_text_background_color)
    ...
    def file_open(self):
        ...
        if path:
            try:
                ...
                self.path = path
                self.orgin_text_length = len(self.editor.toPlainText())
    ...
    def new_file(self):
        if self.path:
            if self.orgin_text_length != self.modified_text_length:
                button_reply = QMessageBox.question(self, 'PyQt5 message', "Do you want to save?",
    ...
    def text_changed(self):
        self.modified_text_length = len(self.editor.toPlainText())

    @staticmethod
    def _get_color():
        color = QColorDialog()
        selected_color = color.getColor()
        print(selected_color.isValid())
